...
type_class::instantiator constant_class_instance(type_ptr t)
{
    return [=](const vector<type_ptr> &){ return t; };
}
...
built_in_types::built_in_types()
{
    ...
    m_integral->instances.push_back(constant_class_instance(m_integer32));
    m_integral->instances.push_back(constant_class_instance(m_integer64));

    m_real = shared(new type_class("Real"));
    m_real->instances.push_back(constant_class_instance(m_real32));
    m_real->instances.push_back(constant_class_instance(m_real64));

    m_complex = shared(new type_class("Complex"));
    m_complex->instances.push_back(constant_class_instance(m_complex32));
    m_complex->instances.push_back(constant_class_instance(m_complex64));
