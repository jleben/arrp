...
private:
    ...
    type visit_cases(const shared_ptr<case_expr> & cexpr) override;
    type visit_array(const shared_ptr<array> & arr) override;
    type visit_array_patterns(const shared_ptr<array_patterns> &) override;
    ...
    type visit_primitive(const shared_ptr<primitive> & prim) override;
